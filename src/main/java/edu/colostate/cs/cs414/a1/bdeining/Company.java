...
public class Company {
    ...
    /**
     * Creates a company instance and sets its name. A valid name is a non-empty string that consists of at
    ...
    /**
     * Adds a worker w to the pool of available workers, if the worker is not already there and if the worker
    ...
    /**
     * Creates a new project with the given a valid name, qualifications and size, and marks the project as
     * PLANNED. A valid name is a non-empty string that consists of at least one non-blank character. Adds
     * the new project to the projects carried out by the company. Adds each qualification in qs to the
     * project’s qualifications.
     *
     ...
     */
    public Project createProject(String n, Set<Qualification> qs, ProjectSize size) throws InvalidName, InvalidQualification {
        ValidationUtils.validateName(name);
    ...
    /**
     * Assigns a worker w to the project p. Only a worker from the pool of available workers can be assigned
     * to a project, as long as the worker is not already assigned to the same project. The project must not
     * be in the ACTIVE or FINISHED state. The worker should not get overloaded by the assignment. The
     * worker can be added iff the worker is helpful to the project. If all the conditions are satisfied, (i) the
     * assigned worker is added to the pool of assigned workers of the company unless the worker is already
     * present in that pool, and (ii) the worker is also added to the project. This results in at least one
     * previously unmet required qualification of the project being met.
     * Note that the same worker can be in both the available pool and assigned pool of workers at the same
     * time. However, the worker cannot be in the assigned pool if the worker is not in the available pool.
     ...
     * unassigned from it. If this was the only project for the worker, then this worker needs to be removed
     * from the pool of assigned workers of the company. If the qualification requirements of an ACTIVE
     * project are no longer met, that project is marked as SUSPENDED. A PLANNED or SUSPENDED project
    ...
    /**
     * Removes a worker w from all the projects that are assigned to the worker. It also removes the worker
     * from the pool of assigned workers of the company. The state of the affected projects must be changed
     * as needed.
     *
     * @param w - the worker to be unassigned.
     * @throws NullPointerException - if w is null
    ...
    /**
     * Starts a PLANNED or SUSPENDED project as long as the project's qualification requirements are all
     * satisfied. The project shifts to an ACTIVE status. Otherwise, the project remains PLANNED or
