...
public class Worker {
    ...
    /**
     * Creates a new worker with the given a valid name and qualifications. A name is valid if it is a non-empty
     * string that consists of at least one non-blank character. Default salary is 0.
     *
    ...
    /**
     * Two Worker instances are equal iff their names match. Note that it
     * is good practice to override the hashCode method when equals is overridden.
     *
     ...
     * qualifications, colon, salary. For example, a worker named "Nick", working on 2 projects, and having
     * 10 qualifications and a salary of 10000 will result in the string Nick:2:10:10000.
     *
    ...
    /**
     * Verifies if the worker will overload by assigning him to a project p. A constraint for the entire system
     * is that no worker should ever be overloaded. To determine overloading, consider all the ACTIVE
     * projects of the worker. If adding a new project p to the current project set of the worker makes
     * (3*numberOfLargeProjects + 2*numberOfMediumProjects + numberOfSmall
     * Projects) greater than 12 when p becomes active, then the worker will be overloaded.
     *
     * @param p
    ...
    public boolean willOverload(Project p) {
        Set<Project> activeProjects = projects.stream().filter(project -> project.getStatus().equals(ProjectStatus.ACTIVE)).collect(
                Collectors.toSet());
